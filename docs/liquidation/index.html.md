...
Liquidation is determined by *[liquidation collateral factors](/helper-functions/index.html.md#get-asset-info-by-address)*, which are separate and higher than borrow collateral factors (used to determine initial borrowing capacity), which protects borrowers & the protocol by ensuring a price buffer for all new positions. These also enable governance to reduce borrow collateral factors without triggering the liquidation of existing positions.

When an account's borrow balance exceeds the limits set by liquidation collateral factors, it is eligible for liquidation. A liquidator (a bot, contract, or user) can call the *[absorb](/liquidation/index.html.md#absorb)* function, which relinquishes ownership of the accounts collateral, and returns the value of the collateral, minus a penalty (*[liquidationFactor](/helper-functions/index.html.md#get-asset-info-by-address)*), to the user in the base asset. The liquidated user has no remaining debt, and typically, will have an excess (interest earning) balance of the base asset.

Each absorption is paid for by the protocol's reserves of the base asset. In return, the protocol receives the collateral assets. If the remaining reserves are less than a governance-set *[target](/liquidation/index.html.md#target-reserves)*, liquidators are able to *[buy](/liquidation/index.html.md#buy-collateral)* the collateral at a *[discount](/liquidation/index.html.md#ask-price)* using the base asset, which increases the protocol's base asset reserves.
...
Reserves are generated in two ways: the difference in interest paid by borrowers, and earned by suppliers of the base asset, accrue as reserves into the protocol. Second, the [liquidation](/liquidation/index.html.md#liquidation) process uses, and can add to, protocol reserves based on the [target reserve](/liquidation/index.html.md#target-reserves) level set by governance.
...
This immutable value represents the target amount of reserves of the base token. If the protocol holds greater than or equal to this amount of reserves, the *[buyCollateral](/liquidation/index.html.md#buy-collateral)* function can no longer be successfully called.
