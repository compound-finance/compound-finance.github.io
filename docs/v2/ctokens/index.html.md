...
Each asset supported by the Compound Protocol is integrated through a cToken contract, which is an [EIP-20](https://eips.ethereum.org/EIPS/eip-20) compliant representation of balances supplied to the protocol. By minting cTokens, users (1) earn interest through the cToken's exchange rate, which increases in value relative to the underlying asset, and (2) gain the ability to use cTokens as collateral.
...
The mint function transfers an asset into the protocol, which begins accumulating interest based on the current [Supply Rate](/v2/ctokens/index.html.md#supply-rate) for the asset. The user receives a quantity of cTokens equal to the underlying tokens supplied, divided by the current [Exchange Rate](/v2/ctokens/index.html.md#exchange-rate).
...
* `RETURN`: 0 on success, otherwise an [Error code](/v2/ctokens/index.html.md#error-codes)
...
The borrow function transfers an asset from the protocol to the user, and creates a borrow balance which begins accumulating interest based on the [Borrow Rate](/v2/ctokens/index.html.md#borrow-rate) for the asset. The amount borrowed must be less than the user's [Account Liquidity](/v2/comptroller/index.html.md#account-liquidity) and the market's available liquidity. To borrow Ether, the borrower must be 'payable' (solidity).
...
Transfer is an ERC-20 method that allows accounts to send tokens to other Ethereum addresses. A cToken transfer will fail if the account has [entered](/v2/comptroller/index.html.md#enter-markets) that cToken market and the transfer would have put the account into a state of negative [liquidity](/v2/comptroller/index.html.md#account-liquidity).
...
A user who has negative [account liquidity](/v2/comptroller/index.html.md#account-liquidity) is subject to [liquidation](/v2/ctokens/index.html.md#liquidate-borrow) by other users of the protocol to return his/her account liquidity back to positive (i.e. above the collateral requirement). When a liquidation occurs, a liquidator may repay some or all of an outstanding borrow on behalf of a borrower and in return receive a discounted amount of collateral held by the borrower; this discount is defined as the liquidation incentive.
...
* `borrower`: The account with negative [account liquidity](/v2/comptroller/index.html.md#account-liquidity) that shall be liquidated.
...
| Event | Description |
|-------|-------------|
| `Mint(address minter, uint mintAmount, uint mintTokens)` | Emitted upon a successful [Mint](/v2/ctokens/index.html.md#mint). |
| `Redeem(address redeemer, uint redeemAmount, uint redeemTokens)` | Emitted upon a successful [Redeem](/v2/ctokens/index.html.md#redeem). |
| `Borrow(address borrower, uint borrowAmount, uint accountBorrows, uint totalBorrows)` | Emitted upon a successful [Borrow](/v2/ctokens/index.html.md#borrow). |
| `RepayBorrow(address payer, address borrower, uint repayAmount, uint accountBorrows, uint totalBorrows)` | Emitted upon a successful [Repay Borrow](/v2/ctokens/index.html.md#repay-borrow). |
| `LiquidateBorrow(address liquidator, address borrower, uint repayAmount, address cTokenCollateral, uint seizeTokens)` | Emitted upon a successful [Liquidate Borrow](/v2/ctokens/index.html.md#liquidate-borrow). |
...
A user who borrows assets from the protocol is subject to accumulated interest based on the current [borrow rate](/v2/ctokens/index.html.md#borrow-rate). Interest is accumulated every block and integrations may use this function to obtain the current value of a user's borrow balance with interest.
...
The user's underlying balance, representing their assets in the protocol, is equal to the user's cToken balance multiplied by the [Exchange Rate](/v2/ctokens/index.html.md#exchange-rate).
...
## Supply Rate
At any point in time one may query the contract to get the current supply rate per block. The supply rate is derived from the [borrow rate](/v2/ctokens/index.html.md#borrow-rate), [reserve factor](/v2/ctokens/index.html.md#reserve-factor) and the amount of [total borrows](/v2/ctokens/index.html.md#total-borrows).
...
Reserves are an accounting entry in each cToken contract that represents a portion of historical interest set aside as [cash](/v2/ctokens/index.html.md#cash) which can be withdrawn or transferred through the protocol's governance. A small portion of borrower interest accrues into the protocol, determined by the [reserve factor](/v2/ctokens/index.html.md#reserve-factor).
...
The reserve factor defines the portion of borrower interest that is converted into [reserves](/v2/ctokens/index.html.md#total-reserves).
